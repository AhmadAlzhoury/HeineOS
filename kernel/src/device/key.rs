...
pub enum Scancode {
    ...
    Enter = 0x1c,
    Space = 0x39,
    // Printable keys of the German keyboard layout.
    // They have an entry in the translation tables of the keyboard driver, but were
    // missing here, so `KeyEvent::scancode()` reported `None` for them. Consumers that
    // decide by scancode whether a key produces text (such as the shell line editor)
    // therefore dropped these characters.
    SharpS = 0x0c,
    Acute = 0x0d,
    UDiaeresis = 0x1a,
    ODiaeresis = 0x27,
...
impl TryFrom<u8> for Scancode {
    ...
    fn try_from(value: u8) -> Result<Self, Self::Error> {
        match value {
            ...
            0x0c => Ok(Scancode::SharpS),
            0x0d => Ok(Scancode::Acute),
            ...
            0x25 => Ok(Scancode::K),
            0x26 => Ok(Scancode::L),
            0x27 => Ok(Scancode::ODiaeresis),
            0x28 => Ok(Scancode::ADiaeresis),
            0x29 => Ok(Scancode::Circumflex),
            0x2b => Ok(Scancode::Hash),
            0x2c => Ok(Scancode::Y),
            0x2d => Ok(Scancode::X),
            0x2e => Ok(Scancode::C),
            0x2f => Ok(Scancode::V),
            ...
            0x31 => Ok(Scancode::N),
            0x32 => Ok(Scancode::M),
            0x33 => Ok(Scancode::Comma),
            0x34 => Ok(Scancode::Period),
            0x35 => Ok(Scancode::Minus),
            0x39 => Ok(Scancode::Space),
            0x47 => Ok(Scancode::Home),
            0x48 => Ok(Scancode::Up),
            0x4a => Ok(Scancode::NumpadMinus),
            0x4e => Ok(Scancode::NumpadPlus),
